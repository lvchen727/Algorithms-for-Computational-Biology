(*In this assignment, I implement the Viterbi algorithm. *)
...
decode[observationSeq_, {states_, alphabet_, emissionMatrix_, transitionMatrix_, initialStateProbs_}] := 
	Module[{stateSeq = {}, viterbiMatrix, numStates, numObservations, i = 2, 
		j = Length[observationSeq], currentState, currentStateIndex, temp}, 
		
		(*create the Viterbi dynamic programming matrix*)
		numStates = Length[states];
		numObservations = Length[observationSeq];
		viterbiMatrix = ConstantArray[0,{numObservations, numStates}];
		viterbiMatrix[[1]] = MapThread[#1*#2 &, {initialStateProbs, Transpose[emissionMatrix][[observationSeq[[1]]]]}];
		While[ i <=  numObservations,
			viterbiMatrix[[i]] = MapThread[#1*#2 &, {Map[Max[viterbiMatrix[[i-1]] * #]&, transitionMatrix], Transpose[emissionMatrix][[observationSeq[[i]]]]}];
			i++;
		];
		
		(*do the traceback directly from the Viterbi matrix*)
		currentStateIndex = Position[viterbiMatrix[[j]], Max[viterbiMatrix[[j]]]][[1]][[1]];
		currentState = states[[currentStateIndex ]];
		stateSeq = Prepend[stateSeq, currentState];
		While[ j > 1,
			temp = Log[ viterbiMatrix[[j-1]] * transitionMatrix[[currentStateIndex]] ];
			currentStateIndex = Position[temp, Max[temp]][[1]][[1]];
			currentState = states[[currentStateIndex]];
			stateSeq = Prepend[stateSeq, currentState];
			j--;
		];
